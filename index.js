...
async function receiveDMEvent(event) {
    ...
    async function getDirectMessage() {
        ...
        if (!((messageVar.senderMessage).toLowerCase()).includes(validation.trigger)) {
            return;
        }

        // [CHANGEABLE] Rejects all messages from users below 100 followers and 500 tweets.
        else if (!(usersVar.usersFollowersCount > validation.followers && usersVar.usersStatusesCount > validation.statuses)) {
...
async function replyMessage(sender_id, sender_screen_name, sender_message, tweet_id) {
    ...
    const requestReply = {
        ...
        event: {
            ...
            message_data: {
                text: `Hai @${sender_screen_name}! 👋. menfess kamu berhasil dikirm nih, cek disini ya \n ~👻 https://twitter.com/scarefess/status/${tweet_id}`
            }
            }
        }
        }
...
async function rejectMessage(sender_id, sender_screen_name) {
    ...
    const requestReject = {
        ...
        event: {
            ...
            message_data: {
                text: `Hai @${sender_screen_name}! 👋. Untuk mengirim menfess kamu harus punya ${validation.followers} followers, dan ${validation.statuses} tweets dulu ya \n ~👻`,
            }
            }
        }
        }
